...
module.exports.create = Promise.promisify(function(config, callback) {
  ...
  cursor.path =
    cursor.path ||
    __dirname + path.sep + "services" + path.sep + "orchestrator";
  cursor.config = cursor.config || {};

  cursor = config.services.membership;
  cursor.path =
    cursor.path || __dirname + path.sep + "services" + path.sep + "membership";
  cursor.config = cursor.config || {};
  cursor = cursor.config;

  cursor = config.services.replicator;
  cursor.path =
    cursor.path || __dirname + path.sep + "services" + path.sep + "replicator";
  cursor.config = cursor.config || {};

  cursor = config.services.proxy;
  cursor.path =
    cursor.path || __dirname + path.sep + "services" + path.sep + "proxy";
  cursor.config = cursor.config || {};

  cursor = config.services.health;
  cursor.path =
    cursor.path || __dirname + path.sep + "services" + path.sep + "health";
  cursor.config = cursor.config || {};
