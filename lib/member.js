...
function Member(parameters) {
  ...
  property(_this, 'orchestrator', orchestrator);
  property(_this, '__happnDisconnectHandler', _this.__onHappnDisconnect.bind(_this));
  property(_this, '__happnReconnectHandler', _this.__onHappnReconnect.bind(_this));
...
Member.prototype.connect = function (member) {
  ...
  Happn.client.create(login, function (error, client) {
    ...
    // TODO: move this to after subscribe success
    ...
    client.onEvent('connection-ended', _this.__happnDisconnectHandler);
    client.onEvent('reconnect-scheduled', _this.__happnDisconnectHandler);
    client.onEvent('reconnect-successful', _this.__happnReconnectHandler);
    ...
    client.on('/*', null, _this.__handleClientEvent, _this.__handleClientCallback);
...
Member.prototype.__handleClientEvent = function (data, meta) {

  var self = this;
  var mapper = new Mapper();

  // we need to get hold of the logged in members to this member, so that we can determine which ones are external clients.
  // we need to propagate the event to the external clients ONLY
  mapper.mapMemberEventToPubSubData(data, meta, function (err, result) {
