...
Orchestrator.prototype.prepare = Promise.promisify(function (callback) {
  ...
  try {
    if (this.happn.config.secure) {
      this.loginConfig.config.username = this.happn.config.services.security.config.adminUser.username;
      this.loginConfig.config.password = this.happn.config.services.security.config.adminUser.password;
    }
  } catch (e) {
