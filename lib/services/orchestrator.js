...
Orchestrator.prototype.stabilised = Promise.promisify(function (callback) {
  ...
  if (this.config.stabiliseTimeout !== 0) {
    this.stableTimeout = setTimeout(function () {
      ...
      while (callback = _this.stabiliseWaiting.shift()) callback(error); // jshint ignore:line
...
Orchestrator.prototype.__stateUpdate = function (report) {
  ...
  if (errors.length > 0) {
    ...
    while (callback = this.stabiliseWaiting.shift()) callback(errors[0]); // jshint ignore:line
    return;
  }
  ...
  while (callback = this.stabiliseWaiting.shift()) callback(); // jshint ignore:line
};
