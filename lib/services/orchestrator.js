...
function Orchestrator(opts) {
  ...
  property(this, "healthLogInterval", null);
...
Orchestrator.prototype.initialize = function(config, callback) {
  ...
  if (this.secure) {
    try {
      property(
        this,
        "adminUser",
        this.happn.services.security.config.adminUser
      );
    } catch (e) {
  ...
  property(
    this,
    "__onConnectionFromHandler",
    this.__onConnectionFrom.bind(this)
  );
  property(
    this,
    "__onDisconnectionFromHandler",
...
Orchestrator.prototype.stop = function(options, callback) {
  ...
  clearTimeout(this.healthLogInterval);

  Promise.resolve(Object.keys(members))
    .map(function(name) {
...
Orchestrator.prototype.prepare = Promise.promisify(function(callback) {
  ...
  property(
    this,
    "__removeMembershipHandler",
    this.__onMembershipRemoveMember.bind(this)
  );
  property(
    this,
    "__addMembershipHandler",
    this.__onMembershipAddMember.bind(this)
  );
  ...
  this.happn.services.session.localClient(localLoginConfig, function(
    error,
    client
  ) {
...
Orchestrator.prototype.stabilised = Promise.promisify(function(callback) {
  ...
  if (this.config.stabiliseTimeout !== 0) {
    this.stableTimeout = setTimeout(function() {
      ...
      while ((callback = _this.stabiliseWaiting.shift()) !== undefined)
        callback(error);
...
Orchestrator.prototype.__stateUpdate = function(report = true) {
  var callback,
    _this = this;
  var peerCount;
  var errors = [];
  ...
  if (peerCount < this.config.minimumPeers) {
    if (!this.stableAwaitingMinimumPeers) {
      this.log.warn(
        "requires %d more peers to stabilise",
  ...
  this.healthLogInterval = setInterval(
    this.checkMembersAndLogClusterHealth.bind(this),
    this.config.healthLogInterval ||
      this.happn.services.membership.config.healthLogInterval ||
      10 * 1000
  );
  clearInterval(this.stableTimeout);
...
Orchestrator.prototype.__onDisconnectionFrom = function(data) {
  ...
  if (!member.member) this.__removeMember(member); //Left Swim as Well
};
...
Orchestrator.prototype.checkMembersAndLogClusterHealth = function() {
  ...
  let allNonLoginPeersAreMembers = () => {
    return this.peers
      ? Object.values(this.peers)
          .filter(peer => !peer.memberFromLogin)
          .every(peer => this.members[peer.memberId])
      : true;
  };
  let allPeersAreMembers = () => {
    return this.peers
      ? Object.values(this.peers).every(peer => this.members[peer.memberId])
      : true;
  };

  let allMembersArePeers = () => {
    return this.members
      ? Object.values(this.members).every(member => this.peers[member.name])
      : true;
  };

  if (!allPeersAreMembers()) {
    Object.values(this.peers)
      .filter(peer => !this.members[peer.memberId])
      .forEach(nonMemberPeer => {
        if (!nonMemberPeer.memberFromLogin && !nonMemberPeer.member) {
          //Member is not from login, and swim doesn't have this peer anymore
          this.log.warn(
            `Lost swim connection to Peer ${nonMemberPeer.name}, removing from list of peers`
          );
          return this.__removePeer(nonMemberPeer);
        }
        if (!nonMemberPeer.memberFromLogin && nonMemberPeer.member) {
          this.log.warn(
            `Lost happn connection to swim connected peer: ${nonMemberPeer.name}, will attempt state update `
          );
          shouldUpdateState = true;
        }
        if (nonMemberPeer.memberFromLogin) {
          this.log.warn(
            `Lost happn connection to peer from login : ${nonMemberPeer.name}, attempting reconnection`
          );
          shouldUpdateState = true;
        }
      });
  }
  if (!allMembersArePeers()) {
    //Members that have not properly connected yet
    Object.values(this.members)
      .filter(member => !this.peers[member.name])
      .forEach(nonPeerMember => {
        this.log.info(
          `Member not connected yet : ${nonPeerMember.name}, will attempt state update`
        );
      });
    shouldUpdateState = true;
  }

  if (shouldUpdateState) this.__stateUpdate();

  if (allMembersArePeers() && allPeersAreMembers())
    return this.log.info("Cluster Healthy");
  if (allMembersArePeers() && allNonLoginPeersAreMembers())
    return this.log.warn(
      "Cluster semi-healthy - All Swim connected peers are happn members, some peers from login are disconnected"
    );
  if (!allMembersArePeers() && allPeersAreMembers()) {
    return this.log.warn(
      "Cluster semi-healthy - Some members are still connecting as peers"
    );
  }
  this.log.warn(
    "Cluster Unhealthy: Some swim peers are no longer happn members "
  );
};
