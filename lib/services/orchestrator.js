...
Orchestrator.prototype.initialize = function (config, callback) {
  ...
  property(this, '__onDisconnectionFromHandler', this.__onDisonnectionFrom.bind(this));
...
Orchestrator.prototype.stop = function (options, callback) {
  ...
  // TODO: fix bug in happn, EventEmitter.removeListener is overridden by completely different thing
...
Orchestrator.prototype.__onConnectionFrom = function(data) {
  ...
  console.log('on connection from', data.info.name);
};

Orchestrator.prototype.__onDisonnectionFrom = function(data) {
  if (!data.info) return;
  if (!data.info.clusterName) return;
  if (data.info.clusterName !== this.membership.config) return;
  console.log('on disconnection from', data.info.name);
};
