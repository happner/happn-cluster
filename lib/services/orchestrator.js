...
Orchestrator.prototype.__stateUpdate = function (report) {
  ...
  Object.keys(this.members).forEach(function (memberId) {
    ...
    if (report) {
      _this.log.info('member %s awaiting %s', member.name || member.memberId, missing.join(', '));
    }
...
Orchestrator.prototype.__onConnectionFrom = function (data) {
  ...
  if (!member) {
    ...
    /*
     LOGIN INFO { name: '10-0-0-10_55001', <---- remote name
     clusterName: 'cluster-name',
     memberId: '10.0.0.10:56001',
     url: 'http://10.0.0.10:55001',
     _browser: false,
     _local: false,
     happn: { name: '10-0-0-10_55007' } }
     */
...
Orchestrator.prototype.__defaults = function (callback) {
  ...
  this.config.stableReportInterval = this.config.stableReportInterval || 2000;
  callback();
};
