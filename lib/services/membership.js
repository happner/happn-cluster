...
Membership.prototype.bootstrap = Promise.promisify(function(callback) {
  ...
  this.config.swimAddress = happnUrl;
  ...
  _this.fetchPersistedMembers(function(error, members) {
    ...
    var memberHosts = members
      ...
      .filter(function(host){
        return config.hosts.indexOf(host) == -1;
      });

    var hosts = config.hosts.concat(memberHosts);
    ...
    setTimeout(function() {
      ...
      _this.swim.bootstrap(hosts, function(error) {
        ...
        function addMember(member) {
          ...
          _this.log.debug('has %d other members (%s arrived)', Object.keys(_this.members).length, member.host);
          _this.emit('add', {
...
Membership.prototype.fetchPersistedMembers = function(callback) {
  ...
  this.happn.services.data.get('/_SYSTEM/_CLUSTER/MEMBERS/' + this.config.swimAddress + '/*', callback);
};

Membership.prototype.persistMember = function(member) {
  var _this = this;
  if (!_this.config.persistMembers) return;
  _this.log.info('member persistence switched on, persisting member: ' + member.host);
  _this.happn.services.data.upsert('/_SYSTEM/_CLUSTER/MEMBERS/' + this.config.swimAddress + '/' + member.host, member, function(e) {
