  ...
  if (this.securityChangeset.length > 0) {
    try {
      this.__replicate(
        ...
        this.batchSecurityUpdate(),
        e => {
          this.log.error(
            `unable to replicate security changeset: ${e.message}`
          );
        }
      );
    } catch (e) {
      this.log.error(`unable to replicate security data: ${e.message}`);
      if (!endingProcess) throw e; // fatal issue, replication is not working while process is ending
    }
  }
...
Replicator.prototype.getChangedKey = function(whatHappened, changedData) {
  switch (whatHappened) {
    ...
    case SD_EVENTS.DELETE_USER:
      return changedData.obj._meta.path.replace(
    ...
    default:
      throw new Error(`unknown security data changed event: ${whatHappened}`);
  }
};

Replicator.prototype.batchSecurityUpdate = function() {
  return this.securityChangeset.splice(0).reduce((reduced, change) => {
    if (!reduced[change.whatHappened]) reduced[change.whatHappened] = {};
    reduced[change.whatHappened][
      this.getChangedKey(change.whatHappened, change.changedData)
    ] = {
...
Replicator.prototype.unpackByWhatHappened = function(payload, whatHappened) {
  if (payload[whatHappened] == null) return [];
  return Object.keys(payload[whatHappened]).map(changedKey => {
    return { whatHappened, ...payload[whatHappened][changedKey] };
  });
};
...
Replicator.prototype.unbatchSecurityUpdate = function(payload) {
  ...
  unbatched = unbatched.concat(
    this.unpackByWhatHappened(payload, SD_EVENTS.LINK_GROUP)
  );
  unbatched = unbatched.concat(
    this.unpackByWhatHappened(payload, SD_EVENTS.UNLINK_GROUP)
  );
  unbatched = unbatched.concat(
    this.unpackByWhatHappened(payload, SD_EVENTS.UPSERT_GROUP)
  );
  unbatched = unbatched.concat(
    this.unpackByWhatHappened(payload, SD_EVENTS.UPSERT_USER)
  );
  unbatched = unbatched.concat(
    this.unpackByWhatHappened(payload, SD_EVENTS.PERMISSION_REMOVED)
  );
  unbatched = unbatched.concat(
    this.unpackByWhatHappened(payload, SD_EVENTS.PERMISSION_UPSERTED)
  );
  unbatched = unbatched.concat(
    this.unpackByWhatHappened(payload, SD_EVENTS.DELETE_USER)
  );
  unbatched = unbatched.concat(
    this.unpackByWhatHappened(payload, SD_EVENTS.DELETE_GROUP)
  );
  return unbatched;
};

Replicator.prototype.emitSecurityDataChanged = function(
  payload,
  isLocal,
  origin
) {
