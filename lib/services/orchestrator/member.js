...
Member.prototype.connect = function (member) {
  ...
  this.memberId = member.memberId; // this member's id (remote swim host)
...
Member.prototype.stop = Promise.promisify(function(callback) {
  ...
  if (typeof this.__disconnectSubscriptionId !== 'undefined') {
    client.offEvent(this.__disconnectSubscriptionId);
  }

  if (typeof this.__retryConnectSubscriptionId !== 'undefined') {
    client.offEvent(this.__retryConnectSubscriptionId);
  }

  if (typeof this.__reconnectSubscriptionId !== 'undefined') {
    client.offEvent(this.__reconnectSubscriptionId);
  }

  if (this.connectedTo) return client.disconnect(callback);

  // can't logout/disconnect without remote server, do it manually

  client.pubsub.on('destroy', function() {
    client.session = null;
    client.initialized = false;
    callback();
  });

  client.pubsub.destroy();
});
...
Member.prototype.__onHappnDisconnect = function () {
  ...
  this.connectedTo = false;
  this.log.warn('TODO: (stabilize) disconnectedTo', this.name);
};
