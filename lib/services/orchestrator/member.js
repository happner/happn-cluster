...
Member.prototype.connect = function (member) {
  ...
  this.HappnClient.create(login, function (error, client) {
    ...
    client.on('/*', null, _this.__getClientEventHandler(), _this.__handleClientCallback.bind(_this));
...
Member.prototype.__handleClientCallback = function (error) {
  if (error) {
    this.error = error;
    this.log.fatal('could not subscribe to %s', 'XXXXX', error);
  }

  this.orchestrator.__stateUpdate(); // TODO: once after all subscribes
};
