...
module.exports = class Member {
  constructor(info, orchestrator) {
    ...
    if (this.url === this.orchestrator.url) {
      this.self = true;
    ...
    this.orchestrator.updateState(this);
  }

  static create(address, orchestrator) {
    return new Member(address, orchestrator);
  }
  ...
  updateOwnInfo(info) {
    ...
    [this.address, this.port] = this.url.split(":");
  }
  ...
  async connect(loginConfig) {
    ...
    loginConfig.url = loginConfig.protocol + "://" + this.url;

    this.log.debug("connect to (->) %s", this.url);
    if (!loginConfig.connectTimeout) loginConfig.connectTimeout = 5000;
    let client;
    try {
      client = await this.HappnClient.create(loginConfig);
    } catch (error) {
    ...
    this.orchestrator.u0pdateState(this);
  }
  ...
  async subscribe() {
    ...
    try {
      ...
      await Promise.all(
        this.orchestrator.config.replicate.map(this.__subscribe.bind(this))
      );
      this.subscribingTo = false;
      this.subscribedTo = true;
    } catch (error) {
  ...
  __subscribe(path) {
    ...
    return new Promise((resolve, reject) => {
      this.client.on(
        path,
        null,
        this.__createReplicationEventHandler(),
        error => {
          if (error) {
            this.log.fatal(
  ...
  __onHappnDisconnect() {
    this.log.debug(
      "disconnected/reconnecting to (->) %s/%s",
      this.clusterName,
      this.name
    );
    this.connectedFrom = false;
  ...
  __createReplicationEventHandler() {
    let rePublisher = (data, meta) => {
      ...
      try {
        action = meta.action.substr(
          meta.action.indexOf("/") + 1,
          meta.action.indexOf("@") - 1
        );
        ...
        return __emitReplicationError(
          error,
          "unexpected meta format, cannot replicate"
        );
      }
      ...
      try {
        ...
        return __emitReplicationError(
          error,
          "error doing getRecipients, cannot replicate"
        );
      }
    };
