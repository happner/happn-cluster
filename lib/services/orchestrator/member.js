...
Member.prototype.stop = Promise.promisify(function (callback) {
  ...
  if (this.connectedTo) return this.client.disconnect(cleanupAndCallback);
...
Member.prototype.connect = function (member) {
  ...
  if (!login.config.connectTimeout) login.config.connectTimeout = 5000; //connection timeout, before we forget about the peers existence
