...
Proxy.prototype.start = Promise.promisify(function (callback) {
  ...
  var allowSelfSigned = typeof this.config.allowSelfSignedCerts == 'boolean' ? this.config.allowSelfSignedCerts : false;
  ...
  var opts = {
    ...
    ssl: typeof this.config.keyPath !== 'undefined' ? {
      key: require('fs').readFileSync(this.config.keyPath),
      cert: require('fs').readFileSync(this.config.certPath)
    } : null
  };

  var listening = opts.ssl == null ? format('http://%s:%d', host, port) : format('https://%s:%d', host, port);
